...
> "No te vendo un reporte. Te instalo un sistema que audita tu negocio antes y después — y te dice, con datos, en qué canal ganas y en cuál pierdes."

### El mensaje que sostiene todo
...
**"Ya tengo mi Excel / mi contador, me funciona."**
No se ataca el Excel — se le hace la pregunta que el Excel no responde: ¿en qué canal específico gana dinero el negocio? ¿Está reteniendo o reemplazando clientes? Esas dos preguntas casi nunca tienen respuesta en una PYME artesanal, y son las que abren la conversación.
...
**3. Andamio**
El andamiaje temporal que permite construir la estructura real, y que se queda visible mientras dura la obra — coincide con que la fase 04 (monitoreo) es acompañamiento continuo, no un proyecto que se entrega y desaparece. Metáfora de construcción igual de clara que "Cimiento" pero con más énfasis en el acompañamiento que en el resultado final.
...
**Concepto:** el punto de partida no es "SaaS moderno", es **auditoría / contaduría / bitácora física** — el objeto de referencia es más un libro de cuentas sellado y verificado que un dashboard. La idea visual central es la transición de caos disperso a registro auditable: trazos de tinta de auditor corrigiendo sobre un libro de cuentas, un sello de "verificado", líneas de una hoja contable.

Explícitamente se evita: gradientes morados, Inter/Space Grotesk, la combinación crema+serif+terracota, tarjetas con esquinas redondeadas — todo lo que hoy se lee como "genérico hecho por IA".

**Paleta**
- Tinta / negro cálido `#16140f` — texto y estructura, no negro puro
- Papel `#F2EFE6` — fondo, papel sin estucar, no crema-con-serif
- Panel `#EAE5D6` — un tono debajo del papel, para separar secciones sin usar sombras
- Acento — rojo de sello/tinta de auditor `#C1391F` — solo para marcas de verificación, números que importan y el CTA
- Verificado — verde de libro de cuentas `#2F5233` — uso escaso, solo para estados "correcto/conciliado"
- Líneas — negro cálido al 12-15% de opacidad, como las líneas de una hoja contable

**Tipografía**
- Titulares: una grotesca geométrica con peso y presencia (Archivo) — no Inter, no Space Grotesk
- Datos, etiquetas, navegación, números: una monoespaciada de datos (IBM Plex Mono) — refuerza la idea de "esto son datos reales, no marketing"
- Nada de esquinas redondeadas en ningún componente; líneas finas (hairlines) en vez de sombras; jerarquía por peso tipográfico y por las reglas horizontales de una hoja contable, no por cajas flotantes

**Logo**
Wordmark, no un ícono ilustrado. El nombre en la grotesca geométrica en mayúsculas, con una regla horizontal debajo (como la línea de "total" en una hoja de cálculo contable) y, aparte, un sello circular simple (un círculo con borde grueso, rotado unos grados, con una palomita o las iniciales adentro) que funciona solo como favicon/avatar — la misma idea del sello "auditoría v2 verificada" convertida en marca.
